...
-- >>> 7. twice twice (+1) 0
-- ОШИБКА ТИПА
   -- Выражение `twice twice` требует создания бесконечного типа
   -- `a = a -> a`


-- 8. twice twice twice
-- ОШИБКА ТИПА
   -- Объяснение: Та же проблема с бесконечным типом, что и в предыдущем пункте.
...
-- 1.
-- (a) f1 :: Int -> Int
-- Бесконечно много реализаций, так как `Int` - конкретный тип.
...
-- (c) f3 :: a -> Int
-- Бесконечно много реализаций. Мы не можем использовать значение типа `a` для получения `Int`,
...
-- (b) t2 :: (a -> Bool) -> [a] -> [a]
-- Одна осмысленная реализация. Предикат используется для принятия решения о включении элемента.
t2 :: (a -> Bool) -> [a] -> [a]
t2 = filter
